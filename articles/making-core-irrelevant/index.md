...
Same chain, same 21 million, same proof of work, not a new coin and not a campaign to leave Bitcoin. Bitcoin has an engineering problem. One implementation writes the menu most economic nodes download. That implementation's undocumented behavior is the target every alternative has to chase. That is a single point of failure dressed as decentralization.

Bitcoin Core's review culture, release engineering, and reluctance to rush consensus changes are why the money rules have held. That record is not the dispute. The dispute is informal merge authority on a capturable architecture, concentrated funding, and an exit so costly it does not constrain who merges.
...
The numbers are public. Seventeen years of Bitcoin Core git history show a PR-weighted authorship Gini of about 0.851 historical / 0.834 recent. The top three merger accounts have executed 81.1% of all-time historical merges (50.2% / 82.2% top-1 / top-3 in the 2022+ window of 5,747 PRs). In calendar year 2025, 56% of all merges flowed through one person funded by Brink ([Engineering Impact Report](https://brink.dev/blog/2026/03/26/engineering-impact-report-2025/)). Five people still hold merge access. Core development drew $8.4 million in 2023 against a two trillion dollar market cap. The tables are in [Bitcoin Governance Research](https://github.com/secsovereign/bitcoin-governance-research) and *[Who Controls Bitcoin](/articles/bitcoin-governance)*.

None of this requires a conspiracy. People fund what they understand, hire from networks they trust, and select for demonstrated alignment. The architecture is capturable. Concentration is structural, not a meeting. That makes it harder to argue against and harder to fix. You cannot swap out maintainers and solve the problem. The problem is the monoculture. What keeps the monoculture in place is the absence of a formal consensus specification. That mechanism is [the no-spec moat](/articles/bitcoin-governance#vi-the-no-spec-moat-and-why-it-matters).

Without a spec, any alternative has to reverse-engineer undocumented behavior from Core itself. That ties every alternative to Core by definition. The no-spec moat is not an oversight but the thing that makes the monopoly rebuild itself every time someone tries to compete. The moat dissolves when a credible formal specification exists, and not before.
...
The nodes tell the same story. As of May 2026, only about 16% of nodes were running Core v30 with the higher OP_RETURN limits, which is the version Core actually shipped (source: [Clark Moody dashboard](https://bitcoin.clarkmoody.com/dashboard/), May 2026). More nodes were running pre-v30 software or Knots than were running Core's own controversial change. Knots peaked at roughly 25% of public nodes in September 2025 and settled around 20% as of mid-2026, the last pre-split reading (source: [Coin Dance](https://coin.dance/)). That is demand for a policy alternative, not implementation diversity. In 2025 that client was Knots, still Core's lineage on Bitcoin. Luke's Knots later followed a BLAKE2b proof-of-work hard fork and left Bitcoin. Core-derived forks that stayed on Bitcoin are still that lineage, which is why *[Who Controls Bitcoin](/articles/bitcoin-governance)* treats reachable-node splits as demand, not as a break in the monoculture. No dated measurement exists for what share stayed after the split. The missing piece is not demand but a separately written client that an exchange or payment processor can actually run, and a clear answer to who is liable if that client splits the chain.
...
Bitcoin Commons is the leading attempt at that path, a from-scratch Rust node written separately from Core, not copied from it.

The Orange Paper is the document that node is built from, Bitcoin's consensus rules written as mathematics a person can read. Signature checks, UTXO accounting, script, and block structure are defined as objects with precise rules instead of as whatever Bitcoin Core happens to do. Any implementation can check itself against that document. That is why the Orange Paper is a public good even if Commons is slow. The no-spec moat dissolves when the spec exists, for the whole ecosystem, not only for Bitcoin Commons.

The BLVM consensus layer is the code that implements those rules, a bounded set of deterministic functions. Wallet, networking, and policy are not mixed into it. That isolation is what makes a proof about the money rules tractable.

The Specification Lock is the check that runs on every merge. It uses Z3, a theorem prover, to ask whether the Rust still satisfies the contracts taken from the Orange Paper. The spec does not write the code. People write the code. If a change would break a consensus property, the merge fails.

Full-chain differential replay against Bitcoin Core is the scale test. `blvm-bench` targets on the order of 900,000 blocks. Those runs are operator-driven and resource-heavy. That is not a claim every CI job has zero-divergence proof to tip. Methodology lives in the [differential testing guide](https://docs.thebitcoincommons.org/development/differential-testing.html). The usual objections about proofs and tests are answered in *[Why Bitcoin Needs a Specification](/articles/why-bitcoin-needs-a-specification)*.

Cryptographic merge authorization is designed and stays off until security review, key management, and community validation. The software carries explicit unreleased warnings. What exists is that foundation. Getting an exchange or payment processor to run it still takes independent security audits, extended mainnet operation, operational hardening, and legal review. That work is ahead, not behind. First institutional adoption is more likely 2028-2029 than late 2027 on an optimistic calendar.
...
Implementation diversity is not only an engineering problem. Core's maintainers and funders have a tight network and a loud story. An alternative needs its own money, a review culture people trust, and a way not to be framed as an attack on Bitcoin. The capturable-architecture critique is correct, and it is also the story Core's defenders will use against any alternative. The answer is not a louder governance argument but a running alternative with a clean audit trail and no chain splits.
...
The formal-spec path needs a release an exchange or payment processor can run. That means an independent security audit, extended mainnet operation without incident, operational hardening, and a prepared answer on liability before the first institutional meeting. The pitch is not a philosophy. A board would need the differential record it can actually read, the Specification Lock that checks the code against the Orange Paper, fuzzing and property-test results, the audit, the insurance, and why this lowers their risk instead of raising it. That packet is not in hand. Finding the people who can have that conversation is a different job from writing the software, and both have to run at once.
...
The implementation monopoly is a solvable engineering problem. The Orange Paper exists. The Specification Lock checks annotated paths. Full-chain differential replay against Core is the scale test, operator-driven, and not a CI-to-tip certificate. Formal verification, fuzzing, differential testing, and property tests are what make a second client safe enough to run. That foundation has not existed before in Bitcoin. What remains is adoption, the liability answer, the audit, and the time those things take inside institutions.
...
- [Bitcoin Commons documentation](https://docs.thebitcoincommons.org). Orange Paper, Specification Lock, differential testing record
