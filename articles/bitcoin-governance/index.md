...
When the same upstream lineage supplies almost all economically relevant consensus nodes, **the distinction between Bitcoin the network and “whatever Bitcoin Core’s maintainers ship” becomes practically meaningless for what operators run.** The money rules are still what every node enforces. The program that holds those rules is still what that lineage releases. This is not a theoretical concern. CVE-2018-17144, an inflation bug that could have allowed Bitcoin to be created out of thin air, sat in production in Bitcoin Core for eighteen months. It sat unused on the chain. Another implementation replaying those blocks would have matched Core. Someone had to read the code to find it. The bug was in Core. **Core's monopoly is what made it dangerous.** Common defenses of that monopoly are addressed in *[Bitcoin Core: The Biggest Fallacies](/articles/bitcoin-core-the-biggest-fallacies)*.
...
Brink's own [Engineering Impact Report 2025](https://brink.dev/blog/2026/03/26/engineering-impact-report-2025/) documents that Michael Ford ("fanquake") merged **56% of changes in calendar year 2025** and led every major release from v28.1 through v30.0, published by the institution paying his salary as a funding success. The **2022+ window (5,747 merged PRs)** in [Bitcoin Governance Research](https://github.com/secsovereign/bitcoin-governance-research) shows the same shape (**50.2%** top-1, **70.4%** top-two, **82.2%** top-3; [`findings/GOVERNANCE_FRAMES.md`](https://github.com/secsovereign/bitcoin-governance-research/blob/master/findings/GOVERNANCE_FRAMES.md)). Those three percentages are not interchangeable. All-time top-three is 81.1%. 2022+ top-1 is 50.2%. Brink 2025 is 56%.

Fanquake also nominated Gloria Zhao for maintainership in June 2022, six months after Zhao had publicly endorsed expanding his GitHub owner permissions, including the ability to block accounts.

Since 2022 the named merger counts are fanquake 2,884, achow101 1,162, maflcko 676, glozow 313, sedited 262. Merge data keys on GitHub logins, which are unique.

Share of each author's merged work, since 2022, that passed through fanquake: theuni 73.3%, hebasto 72.7%, fanquake (self) 68.3%, willcl-ark 62.7%, maflcko 62.6%, sedited 53.7%, achow101 52.3%, glozow 52.0%. These are observational counts. Nobody is an appointed deputy. The MAINTAINERS file is a roster. The merge graph is the org chart.

Self-merge is **25.5%** of maintainer-authored merges. **45.1%** of those carry zero review, which is **11.6%** of all maintainer merges. Non-maintainers self-merge at 0%, being unable to. The spread is the argument. laanwj 77.1%, fanquake 50.4%, maflcko 35.0%. Five maintainers with over 1,200 merges between them have never self-merged. The "ordinary workflow" defense has to explain why most maintainers decline.

Merge authority decides what enters the codebase. Signing authority decides what users install. Of 339 releases, 286 were signed and 53 were unsigned. The source report does not explain the 53. Signing data keys on free-text names rather than logins, so three UID strings for the same person have to be collapsed. On the 282 published signer rows, Wladimir van der Laan 146 (51.8%), fanquake 55 (19.5%), Gavin Andresen 53 (18.8%), collapsed top-3 90.1%, top-5 98.6%. Four signed releases are unaccounted for in those rows, so the shares use 282 as the denominator. A previously published signer Gini of 0.545 used the unjoined name strings and does not apply to these collapsed counts.

First-PR outsider merge rate is about 29% all-time and about 16% since 2022. Established outsiders with five or more prior merges merge at about 69%. Median merge wait is 93.3 hours for maintainers and 145.4 hours for non-maintainers (1.41x). First review time is 12.3 hours versus 12.4 hours. Raw merge rates of about 80% maintainer against about 55% non-maintainer overstate the gap, because most of it is self-merge privilege rather than differential judgment. Peer-merge rates, excluding self-merges, are about 59% against 54% all-time, widening to about 63% against 51% since 2022.

### What the Quantitative Record Shows

The same research finds extreme merge concentration (authorship Gini ~0.851 historical / ~0.834 recent), high contributor churn (**90.7%** of **7,827** contributors inactive in the last year of the dump), and a pattern of governance paralysis on major protocol improvements that would have reduced dependence on the current institutional infrastructure.

That 90.7% is selection into a thin continuing core, not a dying project. Participants who never authored a PR exit at 95.3%, one-time contributors at 92.9%, PR authors at 81.6%, established authors at 58.1%, maintainers among the established set at 23.8%. High-merge-rate authors still exit at 83.0%, so quality does not retain.

Pair-level reviewer cohesion exists, but the joint-appearance counts are too small to treat as a voting machine ([`findings/CONFLICT_RESOLUTION_ANALYSIS.md`](https://github.com/secsovereign/bitcoin-governance-research/blob/master/findings/CONFLICT_RESOLUTION_ANALYSIS.md), [`findings/EXECUTIVE_SUMMARY.md`](https://github.com/secsovereign/bitcoin-governance-research/blob/master/findings/EXECUTIVE_SUMMARY.md)). Informal channels put topics on the table and GitHub finalizes. That flow was ~0.55 in the SegWit and Taproot years and **0.002** after 2022 ([`findings/GOVERNANCE_FRAMES.md`](https://github.com/secsovereign/bitcoin-governance-research/blob/master/findings/GOVERNANCE_FRAMES.md)). BIP-repo merge power is not Core merge power: **443** unique BIP mergers versus **23** on Core, and only **two** people sit in both top tens. For how paralysis became the durable outcome of the blocksize war, see *[Governance Paralysis Was The Victory](/articles/governance-paralysis-was-the-victory)*; for the stalled-proposals evidence list, see *[What Bitcoin's Stalled Proposals Tell You](/articles/what-bitcoins-stalled-proposals-tell-you)*. Review activity Gini sits near **0.92**. Many people review; almost nobody merges. Contributor accounts in Section III describe how funding dependency and institutional geography further narrow who sustains engagement in that dataset.
...
> The oligarchic structure is not a theoretical concern. It is documented across seventeen years of governance data. Reality overrides theory.
...
<div class="chart-heading">Funding and hosting institutions (§III)</div>
...
This network is not assembled by explicit coordination but by the normal operation of financial incentives, social trust, and institutional gravity. People fund what they understand. They hire from networks they trust. They develop intellectual frameworks that align with the interests of the institutions they work for. No individual needs to be corrupt for the architecture to be capturable. The mechanism is structural, not conspiratorial.

> Power without a name is the most dangerous kind. "Bitcoin isn't governable" is the claim that protects the people who already govern it from being held accountable. It is a capturable structure that learned to call itself freedom.

---

## V. The Adversarial Layer: When Conflicts Become Visible

The funding map in Section III is not a static diagram. When a protocol change would benefit institutions already on that map, the same personnel, communication channels, and informal merge authority can move faster than the structure moves on improvements that threaten those interests. The 2025–2026 OP_RETURN controversy is the clearest documented case: a relay-policy change that benefited VC-backed base-layer data businesses, traced from a 2023 documentation amendment through merge over broad opposition. For PR-level chronology, see [hodlonaut's CAPTURE series](https://www.citadel21.com/the-merge) and [Antoine Poinsot's account](https://antoinep.com/posts/relay_policy_drama/). For measured blockspace impact, see *[The Achievable Floor, §VI](/articles/the-achievable-floor#vi-the-actual-floor)*. For what that burden costs node operators in dollars, see *[Full Cost of Running a Bitcoin Node](/articles/full-cost-of-running-a-bitcoin-node)*.

### The OP_RETURN Arc

**Documentation and framing (2023).** In June 2023, Marco Falke (newly funded by OKCoin and Paradigm) filed PR #27832, clarifying that `-datacarriersize` measures serialized `scriptPubKey` size. The PR body does not mention inscriptions. Witness and script-path inscription fields falling outside the setting is the effect. Falke filed it; AJ Towns and Greg Sanders ACKed; Gloria Zhao reviewed it; fanquake merged it over a NACK from Peter Todd. The change was not surfaced in Bitcoin Optech's recap podcast the week it merged, hosted by Brink's Executive Director. That same month, Zhao and Murch published an Optech series arguing that on-chain data publication cannot be prevented and should not live permanently in the UTXO set. The rhetorical frame deployed against Luke Dashjr's filter patch in 2024 was published before the patch existed.
...
[Bitcoin Governance Research](https://github.com/secsovereign/bitcoin-governance-research) finds that closed-unmerged PRs most often die from **non-engagement**, not explicit NACK: the plurality close with **no formal review**, and in a high-prep outsider sample roughly **45%** never received one ([`findings/MAINTAINER_PREMIUM_REPORT.md`](https://github.com/secsovereign/bitcoin-governance-research/blob/master/findings/MAINTAINER_PREMIUM_REPORT.md), [`findings/EXECUTIVE_SUMMARY.md`](https://github.com/secsovereign/bitcoin-governance-research/blob/master/findings/EXECUTIVE_SUMMARY.md)). Silence is a governance outcome.
...
Bitcoin's governance problem is not that bad people made bad decisions. **It is that the architecture is capturable regardless of who occupies the seats.** The mechanism operates through four interlocking components. Documented outcomes sit on that architecture: moderation of named conflicts, stall on one class of change, speed on another.
...
When these conflicts are named in public forums, critics are moderated off. That is not incidental but the behavior of a power structure defending itself. The combination of undisclosed conflicts, concentrated authority, and active suppression of critics is what a capturable architecture looks like in use.

The consequence is paralysis and, when paralysis fails to protect institutional interests, directed action. When the governance structure cannot process improvements that threaten the interests of the institutions funding it, those improvements die in review. [Bitcoin Governance Research](https://github.com/secsovereign/bitcoin-governance-research) documents the pattern in named dossiers: Erlay's full-protocol implementation set shows **0/7** matched PRs merged (scaffolding merges are not delivery; the open thread is a measurement fight, not silence); Dandelion's Core implementation PR closed unmerged after roughly eleven months because stem routing collided with the wallet's own RBF/CPFP and DOS surface ([`findings/STALLED_PROPOSALS_REPORT.md`](https://github.com/secsovereign/bitcoin-governance-research/blob/master/findings/STALLED_PROPOSALS_REPORT.md), [`findings/ARCHIVE_GEMS.md`](https://github.com/secsovereign/bitcoin-governance-research/blob/master/findings/ARCHIVE_GEMS.md)). When the governance structure can be moved to serve those interests (as in the OP_RETURN uncap, commissioned without disclosure by a Chaincode engineer and merged by a Chaincode employee over 21 Concept NACKs) it moves in weeks. **The same structure that stalled a qualified contributor's maintainer nomination for five months through undisclosed private consultations merged the OP_RETURN uncap in 38 days from mailing list post to merge.** The 56% merge concentration is no longer an estimate. It is published by Brink in its own Engineering Impact Report.

Stagnation on throughput, node cost, and payment-layer competitiveness created space centralized alternatives were happy to fill. Stablecoin transaction volume now exceeds Visa annually. Those are use cases Bitcoin should own. The governance structure left Bitcoin effectively unable to compete for them.

Implementation diversity is the structural fix, not a personnel change at Core. **Swapping one small maintainer cohort for another** without changing the monopoly implementation reproduces the same capture vector. For why that shape reconstructs itself from professional habit rather than from who occupies the seats, see *[The Vertical Layer Problem](/articles/the-vertical-layer-problem)*. The fix requires multiple independent implementations with proven consensus compatibility and different governance models, built from formal specifications that make the no-spec moat obsolete. That is not a small ask but exactly as hard as the governance problem predicts it would be.
...
- *[The Coordination Gap](/articles/bitcoins-hidden-crisis)*: social coordination vs technical consensus
...
- [Bitcoin Governance Research, How to cut this corpus](https://github.com/secsovereign/bitcoin-governance-research/blob/master/findings/GOVERNANCE_FRAMES.md)
- [Bitcoin Governance Research, Archive gems](https://github.com/secsovereign/bitcoin-governance-research/blob/master/findings/ARCHIVE_GEMS.md)
