...
A fight about the default client should not mean leaving the chain. Knots Classic left Core's defaults and stayed on Bitcoin, and it is still Core's code. An independently written client, checked against a written specification rather than Core's undocumented behavior, is the missing exit.
...
"Don't trust, verify" holds for a block once the node is running. Almost nobody verifies a Core release the same way. There is no second implementation to check Core against, and no written specification that makes that check a pass or fail. The trustless layer sits on a trusted one. If the identifiable people who write that software fold under pressure, the software layer folds regardless of what the cryptography says. Cheap exit on the same chain is what makes that pressure fail. More implementors do not automatically resist. Additional clients make refusal a real option instead of a delay.

Authorship will never be as even as validation. Most participants will never write that code. That gap is why the people who write the software have more power than the people who run it, and why "the operators will catch it" is not a check. Uneven authorship still does not put merge authority in one seat.

---

## The contradiction

A node can refuse a release, and in practice there is nowhere to go. There is no independently written client an operator can switch to. Knots Classic remains a Core fork, so it is not a destination. You can pin an old Core version for a while, and you cannot pin it forever. Unmaintained node software rots. Bugs pile up, the network moves, and eventually you take the next download or you fall behind. Shipping code and changing the money are still not the same act. The people who write the default still govern, because refusal only delays the next download.

After the OP_RETURN relay change merged, Bitcoin Knots went from about 4.7% of reachable nodes at the end of April 2025 to a peak of 25.45% on September 14, 2025, and stayed above 19% through that stretch, according to [Coin Dance](https://coin.dance/). That was a policy protest on the same codebase, still one client. It did not give anyone a place to stay. Luke's Knots later left Bitcoin on a BLAKE2b proof-of-work hard fork. Knots Classic is the Bitcoin-staying continuation, still Core's lineage. *[Making Core Irrelevant](/articles/making-core-irrelevant)* treats the reachable-node split as demand for a policy alternative, not as a break in the implementation monoculture.
