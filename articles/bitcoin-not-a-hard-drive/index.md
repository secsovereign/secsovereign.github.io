...
`OP_RETURN` was introduced as a controlled exit valve for provably unspendable outputs, ones that cannot be mistaken for monetary UTXOs and do not need to be tracked as potential future spends. Peter Todd proposed it on bitcoin-dev on 2013-02-12 as the honest unspendable marker. Jeff Garzik, on 2014-02-24, described the 0.9 change that made a small metadata output standard as damage control against MasterCoin-style unspendable outputs bloating the UTXO set. He rejected the reading that Core developers were endorsing data storage. Eighty bytes because hashes plus a tag. Luke Dashjr, on 2014-11-16, treated relay size as miner policy, not a Core development topic.

Consensus never capped the field. The current default is `MAX_OP_RETURN_RELAY = MAX_STANDARD_TX_WEIGHT / WITNESS_SCALE_FACTOR`, which is 100,000 vbytes, with `DEFAULT_ACCEPT_DATACARRIER` true. The original relay limit was friction that kept the channel aligned with its purpose, marking outputs as definitively unspendable without turning them into subsidized bulk storage.
...
*Companion to [The Achievable Floor](/articles/the-achievable-floor) (channel taxonomy), [Full Cost of Running a Bitcoin Node](/articles/full-cost-of-running-a-bitcoin-node) (operator cost model), [Why Shitcoins Are Shitcoins](/articles/why-shitcoins-are-shitcoins) (altcoin monetary-asset case), [Who Controls Bitcoin](/articles/bitcoin-governance) (governance evidence), [Bitcoin Governance: Argument Map](/articles/bitcoin-governance-argument-map) (numbered arguments), and the three-BIP stack: [Permanent Data Channel Closure](/bips/permanent-data-channel-closure), [Static Per-Output Miner Fee](/bips/static-per-output-miner-fee), [Dynamic Escalation of the Per-Output Miner Fee](/bips/dynamic-escalation-per-output-fee).*
