...
UTXO Commitments (~2014): replaced with AssumeUTXO, a weaker hardcoded hash solution
...
UTXO Commitments date to at least 2014. If the current state of all unspent outputs is committed in a verifiable way, a new node can sync to that commitment and verify it against the existing proof-of-work chain rather than replaying over a decade of transaction history. Initial block download currently takes days to weeks. A working UTXO commitment scheme gets that to hours or less, with the same security assumptions you already rely on.

What shipped instead is AssumeUTXO, a hardcoded hash of the UTXO set baked directly into the Core source code. A node using AssumeUTXO trusts that hash during the most security-sensitive period of its existence, then validates the full history in the background. The trust assumption is not in the proof-of-work chain. It is in the Core release process and the developers who decide what hash gets hardcoded. That is a weaker security model than a cryptographic commitment scheme, and it still took a decade to arrive.
